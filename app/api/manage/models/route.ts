...
export async function POST(req: NextRequest) {
    try {
        ...
        const { account, model_name, litellm_params, model_info, daily_request_limit } = body;

        if (!model_name || !litellm_params || typeof daily_request_limit !== 'number') {
            return NextResponse.json({ success: false, error: "Missing required fields" }, { status: 400 });
        }

        const newModel = await addManagedModel(account, model_name, litellm_params, model_info || {}, daily_request_limit);
